...
public:
	...
	double longueur() {
		return sqrt(pow(this->p2.getpx()-this->p1.getpx(),2) 
			+ pow(this->p2.getpy()-this->p1.getpy(),2));
	}
	bool estVertical() {
	...
	bool esSurDiagonal() {
		if(p1.getpx() != p2.getpx() && p1.getpy() != p2.getpy()) 
			return true;
		else 
			return false;
	}
...
int main() {
	...
	p2.afficher();
	s.afficherSegment();
	cout<<"V: "<<s.estVertical()<<" D: "<<s.estSurDiagonal()<<" H: "<<s.estHorizontal()<<endl;
